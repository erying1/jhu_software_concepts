...
def clean_data(raw_records: List[Dict]) -> List[Dict]:
    """
    Full cleaning pipeline - LLM DISABLED FOR SPEED
    """
    print(f"Cleaning {len(raw_records)} records...")
    ...
    # 3. SKIP LLM - just copy fields (INSTANT!)
    print("Skipping LLM processing (using original fields)...")
    for rec in cleaned_basic:
        rec["llm-generated-program"] = rec["program_name"]
        rec["llm-generated-university"] = rec["university"]
