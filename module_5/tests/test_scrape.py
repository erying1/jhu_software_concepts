...
@pytest.mark.integration
@patch("src.module_2_1.scrape.urllib.robotparser.RobotFileParser")
...
def test_fetch_detail_batch(mock_parse):
    ...
    records = [{"entry_url": "http://test.com/1"}, {"entry_url": "http://test.com/2"}]
...
@pytest.mark.integration
@patch("src.module_2_1.scrape.check_robots")
@patch("src.module_2_1.scrape._get_html")
...
def test_parse_detail_gre_total_calculation():
    ...
    with patch("src.module_2_1.scrape._get_html", return_value=html):
