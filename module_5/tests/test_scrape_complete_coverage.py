...
class TestScrapeDetailedCoverage:
    ...
    def test_fetch_detail_batch_with_string_urls(self, mock_parse):
        ...
        result = fetch_detail_batch(
            ["http://test1.com", "http://test2.com"], max_workers=2
        )
    ...
    def test_fetch_detail_batch_with_exceptions(self, mock_parse):
        ...
        mock_parse.side_effect = [{"gpa": 3.5}, Exception("Parse error")]

        records = [{"entry_url": "http://test1.com"}, {"entry_url": "http://test2.com"}]
...
if __name__ == "__main__":
    pytest.main(
        [__file__, "-v", "--cov=src.module_2_1.scrape", "--cov-report=term-missing"]
    )
