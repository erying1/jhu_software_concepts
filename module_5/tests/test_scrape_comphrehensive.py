...
def test_parse_detail_american_citizenship():
    ...
    with patch("src.module_2_1.scrape._get_html") as mock_get:
        # Test "american"
        mock_get.return_value = (
            "<html><body><div>Citizenship: American</div></body></html>"
        )
        result = parse_detail_page_html("http://test.com", base_url="http://test.com")
        ...
        # Test "domestic"
        mock_get.return_value = (
            "<html><body><div>Status: Domestic Student</div></body></html>"
        )
        result = parse_detail_page_html("http://test.com", base_url="http://test.com")
        assert result["citizenship"] == "American"
        ...
        # Test "u.s"
        mock_get.return_value = (
            "<html><body><div>Citizenship: U.S. Citizen</div></body></html>"
        )
        result = parse_detail_page_html("http://test.com", base_url="http://test.com")
        assert result["citizenship"] == "American"
...
def test_parse_detail_gpa_exception(monkeypatch):
    ...
    monkeypatch.setattr("src.module_2_1.scrape._get_html", lambda *a, **k: html)
